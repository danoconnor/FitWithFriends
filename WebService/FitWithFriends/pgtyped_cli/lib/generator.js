    ...
    for (const typeDec of typeDecs) {
        ...
        typeDeclarations +=
            `/**\n` +
                ...
                ` * \`\`\`\n` +
                ` */\n`;
        typeDeclarations += `export function ${typeDec.query.name}(params: ${typeDec.query.paramTypeAlias}): Promise<Array<${typeDec.query.returnTypeAlias}>> {\n`;
        typeDeclarations += `  return import('@pgtyped/runtime').then(pgtyped => {\n`;
        typeDeclarations += `    const ${typeDec.query.name} = new pgtyped.PreparedQuery<${typeDec.query.paramTypeAlias},${typeDec.query.returnTypeAlias}>(${typeDec.query.name}IR);\n`;
        typeDeclarations += `    return ${typeDec.query.name}.run(params, DatabaseConnectionPool);\n`;
        typeDeclarations += `  });\n`;
        typeDeclarations += `}\n\n\n`;
    }
